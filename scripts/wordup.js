...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            var theAnswer = response.results.length;
...
function render() {
    ...
    var letterChips = model.allowedLetters.map(letterChip) //HOW TF does allowed.Letters work?
    $("#allowed-letters").append(letterChips);

    // TODO 11
    // Render the word submissions
    var wordSubmission = model.wordSubmissions.map(wordSubmissionChip) //HOW TF does this work?
    ...
    var disallowedLetters = disallowedLettersInWord(model.currentAttempt); //Please explain
...
function wordSubmissionChip(wordSubmission) {
    var wordChip = $("<span></span>")
        .text(wordSubmission.word) //What's this doing here?
        .attr("class", "tag tag-lg word-submission");

    // if we know the status of this word (real word or not), then add a green score or red X
    if (wordSubmission.hasOwnProperty("isRealWord") && wordSubmission.isRealWord == true) {
        
        var scoreChip = $("<span></span>").text(wordScore(wordSubmission.word)).attr("class", "tag tag-sm bg-primary");
        return wordChip.append(scoreChip)}

    else (wordSubmission.hasOwnProperty("isRealWord") && wordSubmission.isRealWord == false)
    {var scoreChip= $("<span></span>").text("X").attr("class",  "tag tag-sm bg-danger");
    return wordChip.append(scoreChip)}
        // TODO 17
...
function isDisallowedLetter(letter) {
    ...
    var listOfLetters = model.allowedLetters
    var listOfLettersCheck = listOfLetters.indexOf(letter)

    if (listOfLettersCheck == -1){
        return true;
    }
}
...
function containsOnlyAllowedLetters(word) {
    ...
    var disallowedLetterList = disallowedLettersInWord(word)
    if(disallowedLetterList.length <= 0){
    return true;
    }
}
